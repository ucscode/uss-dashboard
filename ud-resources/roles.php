...
class roles {
	...
	*/
	...
	public static function remove( string $title ) {
		...
		if( self::get($title) ) unset( self::$roles[ $title ] );
	...
	private static function adjustPriority( int $priority ) {
		...
		*/
		...
		if( $priority > self::$priorityBreak ) $priority = ( self::$priorityBreak - 1 );
	...
	*/
	...
	private static function probe( $permission ) {
		...
		if( !in_array( $type, ['string', 'array']) ) return [];
		
		if( $type == 'string' ) $permission = [ $permission ];
	...
	*/
	...
	public static function get( ?string $title = null, ?string $key = null ) {
		...
		if( is_null($title) ) return self::$roles;
	...
	*/
	...
	public static function setPermission( string $title, $permissions /* string or array */ ) {
		...
		*/
		...
		if( empty($role) || !$permissions ) return false;
	...
	*/
	...
	public static function assign( $roles ) {
		...
		if( empty(self::$user) ) return self::clear_user( "Cannot assign role(s) to user of type NULL" );
		...
		if( empty($roles) ) return self::clear_user( "Assigned role(s) does not match any existing registered role" );
	...
	*/
	...
	public static function unassign( $roles ) {
		...
		if( empty(self::$user) ) return self::clear_user( "Cannot unassign role(s) for user of type NULL" );
	...
	*/
	...
	public static function hasPermission( string $permission ) {
		...
		if( empty(self::$user) ) return self::clear_user( "Cannot detect permission for user of type NULL" );
