...
abstract class udash_abstract {
	 ...
	 * @var string
	 */
	const VIEW_DIR = UDASH_MOD_DIR . "/ud-view";
	 ...
	 * @var string
	 */
	const RES_DIR = UDASH_MOD_DIR . "/ud-resource";
	 ...
	 * @var string 
	 */
	const ASSETS_DIR = UDASH_MOD_DIR . "/ud-assets";
	 ...
	 * @var string
	 */
	const AJAX_DIR = UDASH_MOD_DIR . "/ud-ajax";
	 ...
	 * @var string
	 */
	const PAGES_DIR = UDASH_MOD_DIR . "/ud-pages";
	...
	public static function uploadFile( $mimeTypes, array $file, string $path, ?string $prefix = null ) {
		...
		if( !is_array($mimeTypes) ) {
			if( !is_scalar($mimeTypes) ) throw new Exception("Argument 1 must be a type of string or array");
			else $mimeTypes = [(string)$mimeTypes];
		};
		...
		$mimeTypes = array_values( array_filter($mimeTypes, function($type) {
			if( !is_scalar($type) ) return false;
			$type = (string)$type;
			return !empty($type);
		}) );
		 ...
		 */
		if( empty($mimeTypes) ) throw new Exception("No MIME type was specified in parameter 1");
		...
		*/
		 ...
		 */
		if( empty($fileMIME) ) throw new Exception( "415 — Unsupported Media Type ($originalMIME)" );
		 ...
		 */
		if( $moved ) return $relativeFilepath;
		...
		<div class='text-center py-4 ud-empty-state container-fluid'>
			...
			<div class='py-4'>
				<?php 
					...
					else if( self::can_be_string($var) ) echo $var; 
				?>
			</div>
		</div>
	<?php }
	...
	public static function password( string $password, ?string $hash = null ) {
		 ...
		 */
		if( is_null($hash) ) return password_hash( $password, PASSWORD_DEFAULT );
		/**
	...
	public static function setAccessToken( ?int $userid, string $key = 'default' ) {
		 ...
		 */
		if( is_null($userid) ) return !self::clearAccessToken( $_SESSION[ $ATKey ], $key );
	...
	private static function clearAccessToken( array &$_SDATA, string $key ) {
		...
		if( isset($_SDATA[ $key ]) ) unset( $_SDATA[ $key ] );
	}
	...
	public static function getAccessTokenUser( string $key = 'default' ) {
		 ...
		 */
		if( empty($data) || empty($data[$key]) ) return;
	...
	public static function countries( ?string $code = null, bool $expand = false ) {
		...
		if( !is_null($code) ) {
			...
			$length = strlen($code);
			if( !in_array($length, [2,3]) ) throw new \Exception( "Invalid Country Code - \"{$code}\"" );
			$isoKey = "iso_{$length}";
		}
		...
		if( is_null($code) ) return $countries;
